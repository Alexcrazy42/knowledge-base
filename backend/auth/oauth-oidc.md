...
OpenIdDict* - таблицы, отвечающие за протокол обмена токенами

OpenIdApplications - приложения/клиенты (id, applicationType, client_id, clientSecret, DisplayName, Permission: grant_types, scopes, endpoints, разрешения, requirements, type)

OpenIdScopes - области доступа (name, description, resources)

OpenIdDictTokens - хранение токенов,  access, refresh, authorization codes (id, applicationId, authorizationId - ссылка на accessToken, subject, type, payload, createionDate, expireDate, Status)

OpenIdDictAuthorizations - согласия пользователей consents (id, applicationId, subject, status, scopes, creationDate, lastModifiedDate)
_____

AspNetUsers - пользователи (id, UserName, email, passwordHash, SecurityStamp, LockoutEnabled)

AspNetRoles - роли (id, name, normalizedName)

AspNetUserRoles - связь пользователей и ролей many-to-many (id, userId, roleId)

AspNetUserClaims - доп утвеждения о пользователе (userId, claimType, claimValue)

AspNetRoleClaims - утверждения для ролей (id, roleId, claimType, claimValue)
