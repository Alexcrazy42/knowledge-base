...
- [ ] e2e через Playwright (`@playwright/test`) на полный сценарий:
      ...
      сброс словом СБРОС;
- [ ] юнит-тесты VM (`KanbanViewModel`) с подменой `DialogService`
      (TestBed + провайдер-заглушка вместо promise-диалогов);
- [ ] помнить нюанс для e2e: кнопка переноса задач называется
      «**Перенести и удалить**» (не «Подтвердить…»), модалки -
      `.overlay > .modal`, поиск - `input.search`, карточки - `.task-card`.
