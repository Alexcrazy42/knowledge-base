...
1. EnviromentRecord - объект, в котором, как свойства хранятся все локальные переменные (а также некоторая другая информаци, такая как значение this)
...
Например, для **say("John")** это выглядит так (выполнение находится на строке помеченной стрелкой)

![](images/call_lexical_enviroment.png)

В процессе вызова функции у нас есть два лексичесикх окружения: внутреннее (для вызываемой функции) и внешнее (глобальное):

1. Внутреннее лексическое окружение соответствует текущему выполнению say

В нем находится одна переменная name, аргумент функции. Мы вызываем say("John"), так что значение переменной name равно "John"

2. Внешнее лексическое окружение - это глобальное лексическое окружение

В нем находится переменная phrase и сама функция

У внутреннего лексического окружения есть ссылка на внешнее outer.

Когда код хочет получить доступ к переменной – сначала происходит поиск во внутреннем лексическом окружении, затем во внешнем, затем в следующем и так далее, до глобального.

В сторогом режиме, если переменная не была найдена, это будет ошибкой. Без строгого режима, для обратной совместимости, присваивание несуществующей переменной создает новую глобальную переменную с таким же именем.

**Как происходит поиск в нашем примере:**

1. Для переменной name, alert внутри say сразу же находит ее во внутреннем лексическом окружении

2. Когда alert хочет получить доступ к phrase, он не находит ее локально, поэтому вынужден обратиться к внешнему лексическому окружению и находит phrase там.

![](images/inner_outer_enviroments_example.png)
...
![](images/return_function1.png)

Отличие заключается в том, что во время выполнения makeCounter() создается крошечная вложенная функция, состоящая всего из одной строки ```return count++``. Мы ее не запускаем, а только создаем.

Все функции помнят лексическое окружения, в котором они были созданы. Технически здесь нет никакой магии: все функции имеют скрытое свйоство ```[[Enviroment]]```, которое хранит ссылку на лексическое окружение, в котором была создана функция:

![](images/return_function2.png)

Таким образом, ```counter.[[Enviroment]]``` имеет ссылку на ```{count:0}``` лексического окружения. Так функция запоминает, где она была создана, независимо от того, где она вызывается. Ссылка на ```[[Enviroment]]``` устанавливается 1 раз и навсегда при создании функции.

Впоследствии, при вызове counter(), для этого вызова создается новое лексическое окружение, а его внешняя ссылка на лексическое окружение берется из counter.[[Enviroment]]:

![](images/return_function3.png)


Теперь когда код внутри counter() ищет переменную count, он сначала ищет ее с собственном лексическом окружении (пустом, так как там нет локальных переменных), а затем в лексическом окружении внешнего вызова makeCounter(), где находит count и изменяет ее.

Переменная обновляется в том лексическом окружении, в котором она существует.

![](images/return_function4.png)
...
В JS все функции являются замыканиями (есть одно исключение), они автоматически запоминают, где были созданы, с помощью скрытого свойства [[Enviroment]], и все могут получить доступ к внешним переменным.
```

**Сборка мусора**

Обычно лексическое окружение удаляется из памяти вместе со всеми внутренними переменными, после завершения вызова функции.

Однако если существует вложенная функция, которая все еще доступна после завершения функции, то она имеет свойство [[Enviroment]], ссылающееся на лексическое окружение. В этом случае лексическое окружение остается доступным даже после завршения работы функции.
...
g = null; // только после этого ссылка потеряется
```

**Оптимизация движка**

Движки JS делают некоторые оптимизации, которые могут привести к сюрпризам. 

Движки, например, могут не сохранять в лексическом окружении неиспользуемые переменные. 
...
function f() {
  ...
  function g() {
    debugger; // value из локального лексического окружения нигде не использовалась, поэтому движок не добавил ее в локальное окружение, поэтому если здесь в debugger вызвать alert(value), выведется значение из внешнего окружения
  }
...
**У var же другое поведение:**

1. Игнорирование блоков любых блоков кроме функций и глобальной области видимость (if, for, while, else ...). Это происходит потому, что раньше в JS блоки кода не имели лексического окружения. 

2. var допускает повторное объявление

3. var обрабатывается в начале запуска функции. Переменные var считаются объявленным с самого начала исполнения функции вне зависимости от того, в каком месте функции реально находятся их объявления (при условии что не находится во вложенной функции)

**IIFE**

Раньше существовал только var, а он не имел блочной области видимости, программисты придумали способ её эмулировать. Этот способ получил название **Immediately-invoked function expressions (IIFE)**
...
Здесь создается и немедленно вызывается Function Expression. Код выполняется сразу и имеет свои локальные переменные.

Скобки для Function Expression нужны потому, что когда JS встречает ```"function"``` в основном потоке кода, он воспринимает это как начало Function Declaration. Но у Function Declaration должно быть имя, так что такой код вызовет ошибку
...
1. В браузере он называется windows, в Node.js - global.

2. Недавно globalThis был добавлен в язык как стандартизированное имя для глобального объекта, которое должно поддерживаться в любом окружении. Он поддерживается во всех основных браузерах.

3. В браузере глобальные функции и переменные должны быть объявлены только как var

4. То же самое касается функция, объявленных с помощью синтаксиса Function Declaration

## Объект функции, NFE

Функцию - «объект, который может делать какое-то действие».
...
**Чего мы здесь добились:**

1. Функция все еще считается Function Expression, а не Function Declaration

2. Имя func позволяет функции ссылаться на себя же

3. Имя func не доступно за пределами функции

Мы можем использовать sayHi вместо func, но проблема в том, что sayHi может измениться и логика Function Expression поломается
...
Когда функция создается с использование new Function, в ее [[Enviroment]] записывается ссылка не на внешнее лексическое окружение, в котором она была создана, а на глобальное
...
Имеет такой же синтаксис, как и setTimeout. Отличие в том, что функция запускается периодически.

Можно создать **вложенный setTimeout**, чтобы гибко управлять периодом запуска. Еще одним плюсом является то, что вложенный setTimeout будет запускать timer только после выполненной внутри func().

## Дектораторы и переадресация вызова, call/apply
